...
class Command(BaseCommand):
    ...
    def handle(self, experiment_id, output_folder_path, **options):
        ...
        for pair in pairs.all():
            ...
            coding_output_filename = "%s/%s__coding.log" % (output_folder_path, filename)

            with codecs.open(coding_output_filename, encoding='utf-8', mode='w') as f:
                print >>f, json.dumps(results, indent=3)

            matrix_output_filename = "%s/%s__matrix.log" % (output_folder_path, filename)
            with codecs.open(matrix_output_filename, encoding='utf-8', mode='w') as f:
                print >>f, json.dumps(matrix_stages, indent=3)
            ...
            for stage_idx in matrix_stages:
                ...
                cohens_alphas.append({
                    'stage': stage_idx,
                    'init': init_alpha,
                    'final': final_alpha
                })

            cohens_alpha_output_filename = "%s/%s__cohens_alpha.log" % (output_folder_path, filename)
            with codecs.open(cohens_alpha_output_filename, encoding='utf-8', mode='w') as f:
                print >>f, json.dumps(cohens_alphas, indent=3)
