...
class Command(BaseCommand):
    ...
    def handle(self, experiment_id, output_folder_path, **options):
        ...
        output_filename = "%s/definition_count_table.csv" % (output_folder_path)
        with codecs.open(output_filename, encoding='utf-8', mode='w') as f:
            ...
            for pair in pairs.all():
                ...
                for user in users:
                    stage_count = [0, 0, 0, 0]
                    definition_change_history = ActionHistory.objects.filter(type="definition:handleDefinitionChanges:save", owner=user).all()
